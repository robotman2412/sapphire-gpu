...
object Ahb3MemAccess {
    ...
    def apply(
        ...
        /** If writeable, use independent write port. If not writeable, raise HRESP on write access. */
        ...
        val rdata = if (writeable && !writeMode) {
            ...
            mem.readSync((ahb.HADDR >> exp) resize mem.addressWidth, ahb.HTRANS(1))
        }
            ...
            when (HSIZE === B(i, 3 bits)) {
                ...
                if (writeable) {
                    for (j <- 0 until cfg.dataWidth / width) {
                        ...
                        wdata(width*(j+1)-1 downto width*j) := ahb.HWDATA(width-1 downto 0)
                    }
                }
                // Generate HRDATA mux.
