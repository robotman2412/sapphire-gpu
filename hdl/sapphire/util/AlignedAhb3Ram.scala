package sapphire.util

// Copyright © 2024, Julian Scheffers, see LICENSE for info

import sapphire._
import spinal.core._
import spinal.core.sim._
import spinal.lib._
import spinal.lib.bus.amba3.ahblite._



/** AHB-lite-3 RAM that supports all naturally aligned accesses. */
class AlignedAhb3Ram(
    cfg:        AhbLite3Config,
    depth:      Int        = -1
) extends Component {
    val io = new Bundle {
        val ahb = slave port AhbLite3(cfg)
    }
    val exp = log2Up(cfg.dataWidth/8)
    val mem = if (depth == -1) {
        Mem(Bits(cfg.dataWidth bits), (1 << cfg.addressWidth) * 8 / cfg.dataWidth)
    } else {
        Mem(Bits(cfg.dataWidth bits), depth)
    }
    val rdata = mem.readSync(
        (io.ahb.HADDR >> exp) resize mem.addressWidth,
        io.ahb.HTRANS(1)
    )
    val HADDR  = RegNext(io.ahb.HADDR)
    val HSIZE  = RegNext(io.ahb.HSIZE)
    val HWRITE = RegNext(io.ahb.HWRITE && io.ahb.HTRANS =/= AhbLite3.IDLE)

    // Defaults (when an unsupported size is used).
    io.ahb.HRESP := True
    io.ahb.HRDATA.assignDontCare
    
    val wmask = Bits(cfg.dataWidth bits)
    val wdata = Bits(cfg.dataWidth bits)
    wmask := B(0, cfg.dataWidth bits)
    wdata.assignDontCare
    mem.write(HADDR, wdata, HWRITE, wmask)
    
    io.ahb.HREADYOUT    := True
    for (i <- 0 to exp) {
        val width = 8 << i
        when (HSIZE === B(i, 3 bits)) {
            // Set HRESP to ERROR if misaligned.
            if (i > 0) {
                io.ahb.HRESP := HADDR(i-1 downto 0) =/= U(0, i bits)
            } else {
                io.ahb.HRESP := False
            }
            // Generate wdata mux.
            for (j <- 0 until cfg.dataWidth / width) {
                when (HADDR(exp-1 downto i) === U(j, exp-i bits)) {
                    wmask(width*(j+1)-1 downto width*j) := B((1<<width)-1, width bits)
                }
                wdata(width*(j+1)-1 downto width*j) := io.ahb.HWDATA(width-1 downto 0)
            }
            // Generate HRDATA mux.
            io.ahb.HRDATA(cfg.dataWidth-1 downto 8<<i).assignDontCare
            io.ahb.HRDATA((8<<i)-1 downto 0) := SpinalMap.list[UInt, Bits](
                HADDR(exp-1 downto i),
                for (j <- 0 until cfg.dataWidth / (8<<i)) yield {
                    j -> rdata((j+1)*(8<<i)-1 downto j*(8<<i))
                }
            )
        }
    }
}

