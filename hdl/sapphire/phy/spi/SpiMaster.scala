...
/** Runtime-configurable SPI master PHY that transfers one byte at a time.
  * Supports 1 to 4 bits, both CPOL/CPHA modes and half- or full-duplex
  * operation. Output pins are first, inputs second; may re-use pins for inputs
  * with certain settings. Holds `sclk` at the level according to the last
  * issued action's CPOL. To change CPOL, issue a dummy clock action between
  * deselecting the last and selecting the next chip.
  */
case class SpiMaster(cfg: SpiCfg = SpiCfg()) extends Component {
    ...
    io.bus.rxData.valid.init(False)
