...
case class TextureWriter(cfg: SapphireCfg) extends Component {
    ...
    val io = new Bundle {
        ...
        /** Clear the pipeline and cache. Use before changing texture being
          * written to.
          */
    ...
    val icalc = new builder.Ctrl(0) {
        ...
        val IDX   = insert(
            io.data.payload.x + io.data.payload.y * io.texture.spec.width
        )
    ...
    val mem0 = new builder.Ctrl(plCfg.normMulStages + plCfg.acalcStage.toInt) {
        ...
        when(
            io.texture.spec.pixfmt.bpp < 8 && (!cacheValid || cacheAddr =/= acalc.ADDR)
        ) {
            ...
            val mask =
                ((U"8'b1" << io.texture.spec.pixfmt.bpp(2 downto 0)) - 1).asBits
            io.mem.HWRITE := True
            io.mem.HTRANS := AhbLite3.NONSEQ
    ...
    val mem1 =
        new builder.Ctrl(plCfg.normMulStages + plCfg.acalcStage.toInt + 1) {
            terminateWhen(io.clear)
