...
object ColorMath {
    ...
    private def lerp(from: UInt, to: UInt, coeff: UInt): UInt =
        from + mul(to - from, coeff)
    ...
    def toGreyscale(v: Color): UInt = {
        val tmp = v.r +^ v.g +^ v.b
        ((tmp * U"7'h55")(15 downto 8) + (tmp >= U"16'h017d").asUInt)(
            7 downto 0
        )
    }

    private def expand(v: UInt, width: UInt): UInt = {
        width.mux(
    ...
    private def extractChannel(fmt: ChannelFormat, from: UInt): UInt =
        expand(from |>> fmt.pos, fmt.width)

    def unpack(fmt: PixelFormat, from: UInt): Color = {
        val ch = for (i <- 0 to 3) yield extractChannel(fmt.channel(i), from)
        fmt.fmtType.mux(
            GREY  -> Color.RGB(ch(0), ch(0), ch(0)),
            GREYA -> Color.RGBA(ch(0), ch(0), ch(0), ch(3)),
            RGB   -> Color.RGB(ch(0), ch(1), ch(2)),
            RGBA  -> Color.RGBA(ch(0), ch(1), ch(2), ch(3))
        )
    }

    private def insertChannel(fmt: ChannelFormat, v: UInt): UInt =
        v |>> (U"7" - fmt.width) |<< fmt.pos
    ...
    def pack(fmt: PixelFormat, color: Color): UInt = {
        ...
        val asGrey = toGreyscale(color)

        val res = U(0, 32 bits)

        when(fmt.fmtType === GREY || fmt.fmtType === GREYA) {
            res := res | insertChannel(fmt.channel(0), asGrey)
        } otherwise {
            res := res | insertChannel(fmt.channel(0), ch(0))
            res := res | insertChannel(fmt.channel(1), ch(1))
            res := res | insertChannel(fmt.channel(2), ch(2))
        }
        when(fmt.fmtType === RGBA || fmt.fmtType === GREYA) {
            res := res | insertChannel(fmt.channel(3), ch(3))
        }
